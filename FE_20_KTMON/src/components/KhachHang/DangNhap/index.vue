...
export default {
    ...
    methods: {
        dangNhap() {
            let code = (window.grecaptcha && typeof window.grecaptcha.getResponse === 'function') ? window.grecaptcha.getResponse() : '';
            if (!code) {
                this.$toast.error("Bạn chưa chọn recaptcha");
            } else {
                // send token using the standard field name expected by many backends
                // (Laravel and many server libs expect 'g-recaptcha-response')
                this.tai_Khoan['g-recaptcha-response'] = code;
                ...
                axios
                    ...
                    .catch((res) => {
                        ...
                        if (res && res.response && res.response.data && res.response.data.errors) {
                            ...
                            this.$toast.error('Có lỗi khi đăng nhập. Vui lòng thử lại.');
                        }
                    })
            }
        },
